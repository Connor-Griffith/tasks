...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (sumTotal, question) =>
            question.published ? sumTotal + question.points : sumTotal,
        0,
    );
}
...
export function sameType(questions: Question[]): boolean {
    ...
    if (questions.every((question) => question.type === firstQuestionType)) {
        return true;
    } else {
        return false;
    }
    ...
    return questions.map((question) => {
        if (question.id === targetId) {
            ...
            if (targetOptionIndex === -1) {
                ...
                targetOptionIndex >= 0 &&
                targetOptionIndex < updated.length
            ) {
...
export function duplicateQuestionInArray(
    ...
    if (index >= 0) {
        ...
        newArray.splice(index + 1, 0, duplicatedQuestion);
    }
    return newArray;
}
